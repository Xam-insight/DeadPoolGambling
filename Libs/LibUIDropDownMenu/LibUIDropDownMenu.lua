...
local function create_MenuButton(name, parent)
	...
	local function icon_OnClick(self, button)
		local buttonParent = self:GetParent() -- workaround
		if not buttonParent then -- workaround
			return
		end
		button_OnClick(buttonParent, button) -- workaround
	end
	...
	if (WoWRetail) then -- workaround
		f.NewFeature = CreateFrame("Frame", name.."NewFeature", f, "NewFeatureLabelTemplate") -- workaround
	end -- workaround
